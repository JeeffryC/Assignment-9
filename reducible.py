...
On my/our honor, Krystal Lemus and Jeeffry Casillas, this
...
def get_longest_words(string_list):
    ...
    if string_list == []:
        return []
    longest_words = []
    for _, word in enumerate(string_list):
...
def main():
    ...
    n_value = length_word * 2
    # determine prime number N that is greater than twice
    # the length of the word_list
    while not is_prime(n_value):
        n_value += 1
    ...
    m_value = int(0.2 * len(word_list))
    while not is_prime(m_value):
        m_value += 1
    hash_memo = [""] * m_value
    ...
    largest_reducible_words.sort()
    for _, reduc_words in enumerate(largest_reducible_words):
        print(reduc_words)
